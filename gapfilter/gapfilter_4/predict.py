"""
加载 GapFilter-4 模型，对 test spot 做形态学引导注意力预测并评估。

对每个 test spot：以 K 个最近 train spot 为键值源，
ŷ_test = base(attn@y) + gate * residual。
多折 ensemble 时对预测取平均。
"""
...
from gapfilter.gapfilter_4.dataset import _infer_feature_key, knn_indices
...
def _find_ckpt_paths(sample_ckpt_dir: Path, fold: Union[str, int]) -> List[Path]:
    ...
    def _best_ckpt(d: Path) -> Path:
        pointer = d / "best_ckpt_path.txt"
        if pointer.is_file():
            p = Path(pointer.read_text().strip())
            if p.is_file():
                return p
        for sub in ("stage2_residual", "stage1_base"):
            cand = d / sub / "best.ckpt"
            if cand.is_file():
                return cand
        cand = d / "best.ckpt"
        if cand.is_file():
        ...
        # 优先 stage2
        stage2 = [c for c in ckpts if "stage2_residual" in str(c)]
        if stage2:
            return stage2[0]
        return ckpts[0]
...
def predict_test_spots(
    ...
    nbr_dist_t = torch.from_numpy(neighbor_dist.astype(np.float32)).to(device)
    ...
    for model in models:
        ...
        for start in range(0, n_test, batch_size):
            ...
            nbr_dist = torch.where(mask, dist, torch.zeros_like(dist))
            nbr_dist = torch.where(
                torch.isfinite(nbr_dist), nbr_dist, torch.zeros_like(nbr_dist)
            )
            parts.append(model(q, nbr_x, nbr_y, mask, nbr_dist).float().cpu().numpy())
...
def predict_one_sample(
    ...
    if not ckpt_root.is_dir():
        raise FileNotFoundError(
            ...
            f"例如 model.ckpt_dir=models/gapfilter_4/..."
        )
    ...
    neighbor_local, neighbor_dist = knn_indices(
        coords[test_idx],
        coords[train_idx],
        n_neighbors=k,
